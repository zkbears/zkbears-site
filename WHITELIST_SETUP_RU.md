...
## 3. Необязательное задание Like + Quote

Сейчас `TARGET_POST_ID` оставлен ненастроенным, поэтому задание скрыто. Проверка перехода на профиль X не использует платный X API: после нажатия `OPEN TASK` и возвращения на сайт задание отмечается выполненным.

Настоящая автоматическая проверка Like + Quote обращается к X API и требует платных Credits. Если она понадобится, добавьте необходимые OAuth scopes в Worker и замените `REPLACE_WITH_ANNOUNCEMENT_POST_ID` на числовую часть ссылки поста.
...
- внутренний идентификатор OAuth-подключения без публичного X-профиля;
- зашифрованные OAuth-токены;
- отметка о переходе по кнопке `OPEN TASK`;
- подтверждение Like + Quote;
- Zcash Unified Address;
- дата финальной отправки whitelist.

Один Zcash-кошелёк нельзя использовать в двух разных записях.
