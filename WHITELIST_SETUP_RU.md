...
## 3. Обязательное задание Like + Repost

Пока `TARGET_POST_ID` не настроен, карточка показывает `POST COMING SOON`, а кошелёк и отправка формы остаются заблокированы. После добавления ID пользователь нажимает `OPEN TASK`, переходит к посту, ставит лайк и делает репост. При возвращении на сайт шаг отмечается выполненным без платного X API.
